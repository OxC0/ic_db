...
const ERROR_PREFIX: &str = "IcDb::read::error - ";

// TODO prefix all errors with IcDb::...or something like that
...
) -> Result<Vec<String>, IcDbError> { // TODO I think I want this to return a list of JSON strings...GraphQL can handle type checking the actual values I hope
...
fn find_field_value_stores_for_inputs(
    ...
    inputs: &Vec<ReadInput>
) -> Result<Vec<FieldValueStore>, IcDbError> {
    // TODO I believe the result in the fold here needs to be mutable for efficiency...not sure, but perhaps
    let temp: Result<Vec<FieldValueStore>, IcDbError> = field_values_store.values().try_fold(vec![], |mut result, field_value_store| {
        ...
        if inputs_match == true {
            ...
            return Ok::<Vec<FieldValueStore>, IcDbError>(result);
        }
        else {
            return Ok::<Vec<FieldValueStore>, IcDbError>(result);
        }
